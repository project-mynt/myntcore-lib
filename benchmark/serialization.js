...
async.series([
  function(next) {
    ...
    for (var i = 0; i < 100; i++) {
      ...
      // uint64le
      var br = new myntcore.encoding.BufferWriter();
      var num = Math.round(Math.random() * 10000000000000);
      br.writeUInt64LEBN(new myntcore.crypto.BN(num));
      buffers.push(br.toBuffer());

      // hashes
      var data = myntcore.crypto.Hash.sha256sha256(new Buffer(32));
    ...
    function readUInt64LEBN() {
      ...
      var buf = buffers[c];
      var br = new myntcore.encoding.BufferReader(buf);
  ...
  function(next) {
    ...
    function myntcoreTest() {
      block1 = myntcore.Block.fromString(blockData);
    }
    ...
    var suite = new benchmark.Suite();
    suite.add('myntcore', myntcoreTest, {maxTime: maxTime});
